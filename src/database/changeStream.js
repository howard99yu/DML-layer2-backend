...
async function run() {
  ...
  for await (const change of changeStream) {
    if (change.fullDocument !== undefined) {
      ...
      console.log(
        "Received data in cloud for user: ",
      ...
      if (data.status === "success") {
        ...
        const txHashes = await util.giveReward(
          web3,
          wallet,
          metadatas,
          change.fullDocument.transactionHash,
          contract,
          walletAddresses
        );
        console.log("Rewarded node providers accordingly: ", txHashes);
        let params =
          "Dear Customer,\n\nThank you for using the Decentralized Machine Learning Platform.\nHere are the links for the updated model and weights for you to download.\nhttps://storage.googleapis.com/file-bucket93/model.json\nhttps://storage.googleapis.com/file-bucket93/model.h5\n\nThank you,\nDML Team";
        let title = "Your Model is Availabe for Download";
        ...
        try {
          ...
          const userInfo = await ticketCrud.getTicket({
            userId: change.fullDocument.userId,
          ...
          try {
            ...
            let params =
              "Dear Customer,\n\nUnfortunately, there has been data mishandling by our node providers while performing machine learning on your data. These providers have been banned and will not operate on our platform anymore.\nThe full amount of your payment has been refunded to your wallet address. Please try uploading your data at a later time. We apologize for the inconvenience.\n\nThank you,\nDML Team";
            let title = "Machine Learning Failed";
            let receiver = change.fullDocument.userId;
            await util.sendEmail(params, title, receiver);
          } catch (err) {
            console.log(err);
